...
const selectedCartListCount = computed(() => {
  return selectedCartList.value.reduce((sum, item) => sum + item.count, 0) //item.count每件商品的数量叠加
})
// 计算选中商品的总金额
// 计算选中总金额
...
<template>
  <scroll-view scroll-y class="scroll-view">
    <!-- 已登录: 显示购物车 -->
    <!-- v-if="memberStore.profile"：如果有用户信息就显示购物车 -->
    <template v-if="memberStore.profile">
      ...
      <view class="cart-list" v-if="cartList.length">
        ...
        <uni-swipe-action>
          ...
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            ...
            <view class="goods">
              <!-- 选中状态：商品购物车选中的状态
              @tap="onChangeSelected(item)"：对购物车商品选中状态进行选择-->
              <text
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar">
        <!-- :class="{ checked: isSelectedAll }"：全选状态框根据单选状态框计算出来
        @tap="onChangeSelectedAll"：点击全选状态全选商品的功能，修改前端和后端 -->
