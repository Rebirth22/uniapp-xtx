...
<template>
  ...
  <view class="navbar" :style="{ paddingTop: safeAreaInsets?.top + 'px' }">
    <view class="wrap">
      <!--  v-if="pages.length：页面栈大于1则返回上一层。
      navigate: 点击后进行页面跳转，可以是同一个小程序内的页面，也可以是其他小程序的页面。
      redirect: 点击后进行页面重定向，关闭当前页面，跳转到应用内的某个页面。
      switchTab: 点击后切换到 tabBar 页面，如果目标页面是 tabBar 页面，则会关闭其他非 tabBar 页面。
      reLaunch: 关闭所有页面，打开应用内的某个页面。navigateBack: 关闭当前页面，返回上一页面或多级页面。
      exit: 退出小程序。-->
      <navigator
        v-if="pages.length > 1"
  ...
  >
    <template v-if="order">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-if="order.orderState === OrderState.DaiFuKuan">
          ...
          <view class="tips">
            ...
            <!-- 倒计时组件 -->
            <!-- :second="order.countdown"：后端返回的接口中有这个倒计时时间数据 -->
            <uni-countdown
        ...
        <template v-else>
          <!-- 订单状态文字 -->
          <!--  {{ orderStateList[order.orderState].text }}：匹配constants.ts中的订单状态显示文字 -->
          ...
          <view class="button-group">
            ...
            <!-- v-if="isDev && order.orderState == OrderState.DaiFaHuo"：是否在开发环境或者状态是否为发货状态 -->
            <view
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        <!-- 待付款状态:展示支付按钮 -->
        <!-- v-if="order.orderState === OrderState.DaiFuKuan"：constants.ts文件枚举定义订单的状态，OrderState里面的属性匹配订单状态 -->
        <template v-if="order.orderState === OrderState.DaiFuKuan">
